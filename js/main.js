(() => {
	...
	const sceneInfo = [
		...
		},
		{
			...
			values: {
				...
				messageB_opacity_out: [1, 0, { start: 0.59, end: 0.63 }],
	...
	function playAnimation() {
		...
		switch (currentScene) {
			case 0:
				let sequance = Math.round(calcValues(values.imageSequence, currentYOffset));
				objs.context.drawImage(objs.videoImages[sequance], 0, 0);
			...
			case 2:
				...
				if (scrollRatio > 0.9) {
					...
					// 스크롤바 포함 넓이 = window.innerHeight
					// 스크롤바 제외 넓이 = document.body.offsetWidth
					const recalculatedInnerWidth = document.body.offsetWidth / canvasScaleRatio + 1;
